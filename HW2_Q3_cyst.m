%%
zone_size=round(length(anecoicCyst_data)/5);

zone1=anecoicCyst_data(1:zone_size,:,:);
zone2=anecoicCyst_data(zone_size+1:2*zone_size,:,:);
zone3=anecoicCyst_data(2*zone_size+1:3*zone_size,:,:);
zone4=anecoicCyst_data(3*zone_size+1:4*zone_size,:,:);
zone5=anecoicCyst_data(4*zone_size+1:end,:,:);
...
for bb=-63.5:1:63.5
    xe_Z1(bb+64.5)=((anecoicCyst.elementSpacingMM)/1000)*bb;
...
for bb=-63.5:1:63.5
    xe_Z2(bb+64.5)=((anecoicCyst.elementSpacingMM)/1000)*bb;
...
for bb=-63.5:1:63.5
    xe_Z3(bb+64.5)=((anecoicCyst.elementSpacingMM)/1000)*bb;
...
for bb=-63.5:1:63.5
    xe_Z4(bb+64.5)=((anecoicCyst.elementSpacingMM)/1000)*bb;
...
for bb=-63.5:1:63.5
    xe_Z5(bb+64.5)=((anecoicCyst.elementSpacingMM)/1000)*bb;
...
for hh=1:128
    temp=interp1(timeArray2(:,hh),squeeze(anecoicCyst_data(:,hh,:)),time_withDelays(:,hh),'linear',0);
